...
export const FilterButtons = ({ onClose, defaultRating, defaultReview }: FilterButtonsProps) => {
  ...
  const [ratingValue, setRatingValue] = React.useState([
    defaultRating ? parseFloat(defaultRating) : 0,
  ]);
  const [reviewValue, setReyValue] = React.useState([
    defaultReview ? parseFloat(defaultReview) : 0,
  ]);
  ...
  return (
    <div className="flex gap-4 p-4">
      <div ref={ratingRef} className="relative">
        ...
        >
          Reyting {(ratingValue[0] ?? 0 > 0) && !ratingOpen ? `(${ratingValue[0]})` : ''}
          {ratingOpen ? (
      ...
      <div ref={reviewRef} className="relative">
        ...
        >
          Rəy {(reviewValue[0] ?? 0 > 0) && !reviewOpen ? `(${reviewValue[0]})` : ''}
          {reviewOpen ? (
            <ChevronUp className="ml-2 h-4 w-4" />
          ) : (
            <ChevronDown className="ml-2 h-4 w-4" />
